# Robust Plugin Uninstall Script
# Automatically removes the MetadataRenamer plugin completely
# This script handles all edge cases including locked files and multiple installation paths
...
Write-Host "=== MetadataRenamer Plugin Uninstall ===" -ForegroundColor Cyan
Write-Host ""
...
if (-not $jellyfinPath) {
    ...
    Write-Host "  .\uninstall-plugin.ps1 -JellyfinPath 'C:\Path\To\Jellyfin\Server'" -ForegroundColor Cyan
    exit 1
}
...
if ($stillExists.Count -gt 0) {
    ...
    Write-Host ""
    Write-Host "  These folders may be locked by another process." -ForegroundColor Yellow
    Write-Host "  Try:" -ForegroundColor Yellow
    Write-Host "    1. Close all Jellyfin-related programs" -ForegroundColor White
...
if ($stillExists.Count -eq 0) {
    ...
    Write-Host "1. Start Jellyfin service (if needed)" -ForegroundColor White
    ...
    Write-Host "4. Click Install to reinstall the plugin" -ForegroundColor White
} else {
